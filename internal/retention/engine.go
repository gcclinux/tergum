...
// 3. Skip files with only one version (single-version files never touched)
// 4. Mark the latest version (first in list) as PROTECTED
// 5. For remaining versions, evaluate against policies (priority order, first-match-wins)
...
func (e *RetentionEngine) Evaluate(ctx context.Context, dryRun bool) (*RetentionResult, error) {
	...
	for _, fp := range filePaths {
		...
		// Skip single-version files (safety rule).
		if len(versions) <= 1 {
			if len(versions) == 1 {
				result.Protected++
			}
		...
		for _, entry := range olderVersions {
			...
			matchedPolicy := findMatchingPolicy(enabledPolicies, entry.FilePath)
...
func shouldExpire(entry model.BackupEntry, policy model.RetentionPolicy, now time.Time, totalVersions int) bool {
	// Check keep_versions: we must keep at least keep_versions versions.
	// The keep_versions count includes the protected latest version.
	// So if totalVersions <= keep_versions, don't expire anything.
	if totalVersions <= policy.KeepVersions {
		return false
	}
